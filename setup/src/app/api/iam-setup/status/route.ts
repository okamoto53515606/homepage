 ...
 *
 * 目的 (why):
 *   UI で「まだ root キーが入っている / すでに homepage-deployer に切り替え済み」を
 *   判定するために使う。STS:GetCallerIdentity は homepage-deployer ポリシーにも
 *   含めているので、root / IAM ユーザーどちらでも呼べる。
 *
 *   ARN に :root が含まれていれば root キー、
 *   :user/homepage-deployer が含まれていれば切り替え済みと判定する。
 */

import { NextResponse } from "next/server";
import { STSClient, GetCallerIdentityCommand } from "@aws-sdk/client-sts";
import { readEnv } from "@/lib/env";
import { HOMEPAGE_DEPLOYER_USER_NAME } from "@/lib/homepage-deployer-policy";
