...
/**
 * setup1c+: IAM ユーザー homepage-deployer を作成し、root アクセスキーと入れ替える。
 *
 * 目的 (why - 初心者向け):
 *   ここまで使ってきた AWS アクセスキーは "root" という最強の権限を持つ鍵で、
 *   うっかり漏らすと AWS アカウントごと壊される危険がある。
 *   これから作るのは「homepage のリソースだけ触れる」専用の鍵 (IAM ユーザー) で、
 *   以降のセットアップも homepage 運用もこれで十分。
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-gray-600">
          ...
          漏洩・誤操作時の影響が大きいです。
          <br />
          homepage 運用に必要な権限だけを持った専用ユーザー（IAM ユーザー）を作って、鍵を差し替えましょう。
        </p>
      </div>
      ...
      <div className="border border-gray-200 rounded-lg overflow-hidden">
        ...
        <ul className="p-4 text-sm text-gray-700 list-disc list-inside space-y-1">
          <li>
            IAM ユーザー{" "}
            <code className="bg-gray-100 px-1 rounded">homepage-deployer</code>{" "}
            を作成
          </li>
          <li>
            homepage 関連のリソース（Cognito / DynamoDB / S3 / Lambda /
        ...
        <div className="bg-blue-50 border-t border-blue-200 px-4 py-3 text-xs text-blue-800">
          ...
          <ul className="mt-1 list-disc list-inside space-y-0.5">
            ...
            <li>
              homepage 以外のリソース（他サービスの EC2 / RDS / VPC 等）の操作
            </li>
          </ul>
          <p className="mt-2">
          ...
          <div className="mt-2 space-y-1">
            ...
            {status.isDeployer && (
              <p className="text-green-700">
                ✓ 既に homepage-deployer ユーザーに切り替え済みです（{status.arn}）。
              </p>
            )}
      ...
      <div className="border border-gray-200 rounded-lg p-4 space-y-3">
        ...
        >
          {loading
            ...
            : alreadyDeployer
              ? "アクセスキーを再発行する"
              : "homepage-deployer ユーザーを作成してキーを切り替える"}
        </button>
