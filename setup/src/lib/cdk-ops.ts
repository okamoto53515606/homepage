...
export const LAMBDA_FUNCTIONS = [
  "homepage-app",
  "homepage-stripe-webhook-proxy",
] as const;

/** プロジェクトルート (homepage/) を絶対パスで返す */
...
/**
 * HomepageDynamoDbStack の Outputs を CloudFormation API から取得する。
...
/**
 * HomepageWafStack の WebACL ARN を取得する（存在しなければ空文字）。
 *
 * why:
...
/**
 * cdk-outputs.json から `HomepageDynamoDbStack` の OutputValue を取り、
...
export async function syncSafeEnvToLambdas(
  ...
  // CLOUDFRONT_DISTRIBUTION_ID は名前的に独自ドメインに見えるが distribution の id
  // であり、独自ドメイン文字列ではないので push してよい。
...
/**
 * `cdk deploy HomepageDynamoDbStack --exclusively` を実行するための
 * context 引数を組み立てる。
 *
 * why:
