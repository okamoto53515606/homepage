...
**個人向けメディアシステム「homepage」セットアップ手順書Part3**（Stripe本番環境のセットアップ）については、後日追加予定です。

本番環境を有効にするには、Stripeの審査が必要になります。
審査では、ビジネス情報や銀行口座情報の登録が求められます。

本番環境が有効になると、実際のお客様からの決済を受け付けられるようになります。お楽しみに！
