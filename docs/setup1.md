...
2027年3月22日をもって、手順で利用しているブラウザ完結型の開発環境「Firebase Studio」が廃止されることになりました。

「非エンジニアでもAIを使ってブラウザ上で完結できる」という便利な環境が失われるのは残念ですが、
Firebase Studio終了後も「homepage」のシステム自体は引き続き動作します
のでご安心ください。

このアナウンスを受け、「homepage」のセットアップ手順を以下のように大幅にアップデートする予定です。

- **VS Code + GitHub Copilot 構成への統一**
- **セットアップシェルの活用による手順の簡略化**
- ブラウザでの手作業を減らし、ターミナルへのコピペ中心で簡単に環境構築ができるように改善

修正版の手順書は、準備が整い次第改めて記事でお知らせいたします。より堅牢で自由度の高い開発環境へステップアップする良い機会と捉え、一緒に乗り越えていきましょう！
