...
4. 作業を終えたらトレイ右クリック → **「停止」**。
5. トレイごと終了したい場合は → **「終了」**（サーバー起動中は確認ダイアログが出る）。
...
- WSL ディストリビューション（`homepage-v2-latest`）も自動で削除される。
- `logs\` 以外のキャッシュも削除される。残りを消したい場合は `%LOCALAPPDATA%\HomepageV2` フォルダごと削除。
...
| SHA256 不一致 | DL 破損。`%LOCALAPPDATA%\HomepageV2\cache` フォルダを削除してインストーラ再実行 |
| ポート 3001 が使用中 | 他アプリを停止してから「起動」。すでに起動中の場合は「ブラウザを開く」のみ動作 |
| 「起動」クリックしてもブラウザが開かない | 60 秒でタイムアウト。トレイ右クリック →「ログフォルダを開く」で `tray.log` を確認 |
| その他動作不良 | トレイ右クリック →「ログフォルダを開く」で `%LOCALAPPDATA%\HomepageV2\logs` の `tray.log` / `install-*.log` を確認 |

> インストーラのビルド・設計の詳細は [homepage-v2-installer リポジトリ](https://github.com/okamoto53515606/homepage-v2-installer) を参照。WSL イメージ自体の構築フローは [docs/wsl.md](./wsl.md) を参照してください。
