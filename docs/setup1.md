...
| WSL2 | 事前に `wsl --install` 済みであること（未セットアップなら下記 0 を先に実施） |
| ネットワーク | GitHub Releases へアクセス可能（初回のみ。以降はオフラインで起動可） |
| ポート | TCP `3001` がローカルで利用可能であること |
| 権限 | 一般ユーザーで OK（管理者権限不要）。ただし `wsl --install` 時のみ管理者権限が必要 |

> **インストール先（参考）:** `%LOCALAPPDATA%\HomepageV2`（`app\` 実行ファイル / `wsl\` VHDX / `cache\` DL 中の一時 / `logs\` ログ）。WSL ディストリ名は `homepage-v2-latest`。

### 0. WSL2 のセットアップ（未インストールの場合のみ）

既に WSL を使っているならスキップして「1. インストール」へ。WSL の有無が分からなければ、念のため以下を実施しても害はありません（既にインストール済みなら「既にインストールされています」と表示されるだけ）。
...
4. **「管理者として実行」** を選択
5. 「ユーザーアカウント制御」が出たら **「はい」**
6. 青い PowerShell ウィンドウが開く（タイトルバーに「管理者:」と表示されることを確認）

#### WSL をインストールする

7. 開いた PowerShell に以下を入力して **Enter**:
...
8. ダウンロードと有効化が始まる（数分）。
9. 完了メッセージが出たら **PC を再起動**。
10. 再起動後、初回ログインで Ubuntu のセットアップ画面が出ることがあるが、本インストーラでは使わないのでそのまま閉じて OK（ユーザー名/パスワードを聞かれた場合は適当に入力して完了させても可）。
...
| 症状 | 対処 |
|---|---|
| `wsl --install` を求められる | 管理者 PowerShell で `wsl --install` → PC 再起動 → インストーラを再実行 |
